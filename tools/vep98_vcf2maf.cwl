...
id: kfdrc-vep98-somatic-annotate-maf
requirements:
  ...
  - class: DockerRequirement
    dockerPull: 'pgc-images.sbgenomics.com/d3b-bixu/vep:r98'
baseCommand: [tar, -xzf ]
arguments:
  - position: 1
    shellQuote: false
    valueFrom: >-
      ...
      ${
        var arg = " --tumor-id " + inputs.tumor_id + " ";
        if (inputs.normal_id != null){
            arg += "--normal-id " + inputs.normal_id + " ";
        }
        return arg;
      }
...
inputs:
  ...
  tumor_id: string
  normal_id: {type: ['null', string], doc: "Enter ID if tumor AND normal are there, else, whether tumor-only or germline-only, leave this blank"}
  tool_name: string
  cache: { type: File, label: tar gzipped cache from ensembl/local converted cache }
  cache_version: {type: ['null', int], doc: "Version being used, should match build version", default: 98}
