...
inputs:
  ...
  vardict_public_vcf: {type: File, secondaryFiles: [.tbi], doc: "VEP annotated vcf file."}
  ...
  vardict_protected_vcf: {type: File, secondaryFiles: [.tbi], doc: "VEP annotated vcf file."}
...
steps:
  ...
  vardict_vcf2maf_protected:
    run: ../tools/kf_mskcc_vcf2maf.cwl
    in:
      reference: reference
      input_vcf: vardict_protected_vcf
      output_basename: output_basename
      tumor_id: tumor_id
      normal_id: normal_id
      tool_name:
  ...
  vardict_vcf2maf_public:
    run: ../tools/kf_mskcc_vcf2maf.cwl
    in:
      reference: reference
      input_vcf: vardict_public_vcf
      output_basename: output_basename
      tumor_id: tumor_id
      normal_id: normal_id
      tool_name:
  ...
  rename_outputs_protected:
    ...
    in:
      input_files:
        source: [strelka2_vcf2maf_protected/output_maf, mutect2_vcf2maf_protected/output_maf, lancet_vcf2maf_protected/output_maf, vardict_vcf2maf_protected/output_maf]
        valueFrom: "${return [ self[0],self[1],self[2],self[3] ]}"
      rename_to:
        ...
        var var_maf=self + '.vardict_somatic.norm.annot.protected.maf'; \
        return [str_maf, mut_maf, lan_maf, var_maf];}"
    out: [renamed_files]
  ...
  rename_outputs_public:
    ...
    in:
      input_files:
        source: [strelka2_vcf2maf_public/output_maf, mutect2_vcf2maf_public/output_maf, lancet_vcf2maf_public/output_maf, vardict_vcf2maf_public/output_maf]
        valueFrom: "${return [ self[0],self[1],self[2],self[3] ]}"
      rename_to:
        source: output_basename
        ...
        var var_maf=self + '.vardict_somatic.norm.annot.public.maf'; \
        return [str_maf, mut_maf, lan_maf, var_maf];}"
    out: [renamed_files]
